  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        ...
        setArticle(null); // Ou redirecionar para uma página de erro/404
      }
    };
    fetchArticle();
  }, [params.slug]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Artigos Populares</h3>
              <div className="space-y-4">
                {/* POPULAR_ARTICLES.map(article => (
                  <Link key={article.id} href={`/artigos/${article.slug}`} className="flex items-center space-x-4 group">
