...
export default function AdminDashboard() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (response.ok) {
        // Recarregar a página atual após a exclusão
        fetchArticles(page);
      } else {
  ...
  // Calcular estatísticas com base em todos os artigos, não apenas os da página atual
  const publishedCount = totalArticles > 0 ? 
    articles.filter(article => article.isPublished).length : 0;
  const draftCount = totalArticles > 0 ? 
    articles.filter(article => !article.isPublished).length : 0;
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white shadow-sm rounded-lg border border-gray-200">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-medium text-gray-900">Artigos</h2>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="mt-1 text-sm text-gray-500">Comece criando seu primeiro artigo.</p>
              <div className="mt-6">
                <button
                  onClick={() => router.push('/admin/articles/new')}
                  className="inline-flex items-center px-4 py-2 bg-primary-900 text-white rounded-lg hover:bg-primary-800"
                >
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                  </svg>
            ...
            <>
              ...
              <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
                <div className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.min(page * 10, totalArticles)}</span> de{' '}
